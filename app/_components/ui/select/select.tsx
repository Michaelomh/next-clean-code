...
// check - https://www.shadcnui-blocks.com/components/select
...
function Select({
  ...
  return <SelectPrimitive.Root data-slot="select" aria-readonly={props.readOnly} {...props} />
}
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      {...props}
    >
      {icon && <div className="flex items-center [&_svg:not([class*='size-'])]:size-4">{icon}</div>}
      {children}
      {selectSide && (
        <div className="relative flex items-center [&_svg:not([class*='size-'])]:size-4">{selectSide}</div>
      )}
      <SelectPrimitive.Icon asChild>
        {dropdownIcon ? dropdownIcon : <TbChevronDown className="size-4 opacity-50" />}
